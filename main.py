...
VERSION = "1.0.0"
TOOLS_NAME = "KURUMI -TOOL"
# [ADDED BY KURUMI] - New Author and Version info for the integrated tool
TOOLS_AUTHOR = "Kurumi"
TOOLS_VERSION = "2.0.0"
CALL_DELAY = 4.8
...
def print_banner(tick=0):
    ...
    title = gradient_text("KURUMI -OTP", tick, 0)
  ...
  ┌──────────────────────────────────────────────────────┐
  │  {color}API{reset}  : 24  │  {color}Version{reset}  : 1.0.0  │  {color}Dev{reset}  : Kurumi     │
  └──────────────────────────────────────────────────────┘{reset}
"""
    print(banner)
    # [ADDED BY KURUMI] - Display updated tool details without altering original banner
    print(f"{Fore.CYAN}┌{'─' * 58}┐{Style.RESET_ALL}")
    print(f"{Fore.CYAN}│{Style.RESET_ALL}  {Fore.MAGENTA}Author{reset} : {Fore.YELLOW}{TOOLS_AUTHOR}{reset}  │  {Fore.MAGENTA}Version{reset} : {Fore.YELLOW}{TOOLS_VERSION} (Call+OTP){reset}  {Fore.CYAN}│{Style.RESET_ALL}")
    print(f"{Fore.CYAN}│{Style.RESET_ALL}  {Fore.GREEN}Info{reset}   : {Fore.WHITE}Integrated Missed Call & OTP Spammer{reset}{' ' * 10}{Fore.CYAN}│{Style.RESET_ALL}")
    print(f"{Fore.CYAN}└{'─' * 58}┘{Style.RESET_ALL}")
...
def print_menu(selected=0, tick=0):
    ...
    # [ADDED BY KURUMI] - Appending Call Spam to menu without deleting any lines
    items.append(("📞 Call Spam", "Citcall Missed Call Spammer (4.8s delay)", color3))
...
def menu_navigation():
    ...
    # [ADDED BY KURUMI] - Appending Call Spam to switch case list
    items.append("call")
    tick = 0
        ...
        try:
            ...
            if is_termux:
                ...
                if key == '\x1b[A':  
                    ...
                    # [ADDED BY KURUMI] - New menu option for Call Spam
                    elif choice == "call":
                        call_spammer()
                ...
                # [ADDED BY KURUMI] - Adding a 4th option without deleting previous print lines
                print(f"{Fore.CYAN}   [4] Call Spam{Style.RESET_ALL}")
                choice = input(f"\n{Fore.WHITE}Pilih (1/2/3/4): {Style.RESET_ALL}").strip()
                ...
                # [ADDED BY KURUMI] - Option 4 calls the Call Spammer
                elif choice == "4":
                    call_spammer()
...
# [ADDED BY KURUMI] - Implementation of the Spam Call function (Citcall)
def call_spammer():
    clear_screen()
    print(f"{Fore.CYAN}╔{'═' * 50}╗{Style.RESET_ALL}")
    print(f"{Fore.CYAN}║{Style.RESET_ALL}  {Fore.WHITE}📞 SPAM CALL BY KURUMI {Style.RESET_ALL}")
    print(f"{Fore.CYAN}╚{'═' * 50}╝{Style.RESET_ALL}\n")
    try:
        no = input("No (format 62xxxx) : ")
        jml = int(input("Count (1-999)      : "))
        print()
    except ValueError:
        print(f"\n{Fore.RED}✗ Only Number!{Style.RESET_ALL}")
        time.sleep(2)
        return
    
    global exec_data
    exec_data['target'] = no
    exec_data['total_api'] = jml
    exec_data['sent'] = 0
    exec_data['success'] = 0
    exec_data['failed'] = 0
    exec_data['status'] = 'Call Spam Running...'
    exec_data['progress'] = 0
    exec_data['last_log'] = 'Initializing Citcall...'
        ...
        while n < int(jml):
            try:
                ...
                if 'Success' in send:
                    ...
                    exec_data['sent'] = n
                    exec_data['success'] += 1
                    exec_data['progress'] = int((n / int(jml)) * 100)
                    exec_data['last_log'] = f"Call sent to {no}"
                else:
                    exec_data['last_log'] = "Limit reached (try again later)"
                    break
            except Exception as e:
                exec_data['last_log'] = f"Error: {str(e)}"
                exec_data['failed'] += 1
                n += 1
                exec_data['sent'] = n
                exec_data['progress'] = int((n / int(jml)) * 100)
                # Wait a second before trying the next one just to be safe
                time.sleep(1)
